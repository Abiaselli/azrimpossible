...
class AstroSelfAttention(nn.Module):
    def __init__(self, hidden_dim, n_heads):
        ...
        self.trace_to_prob = nn.Sequential(
            nn.Linear(1, 16),
            nn.SiLU(),                     # smooth nonlinearity (like bio neurons)
            nn.Linear(16, 8),
            nn.SiLU(),
            nn.Linear(8, 1)
        )
    ...
    def forward(self, x, astro_trace):
        ...
        # --- Trace-Gated Dropout ---
        if self.training:
            # Learnable dropout bias
            #drop_input = torch.clamp(astro_trace, 0, 1)
            log_trace = torch.log1p(astro_trace)  # safer variant of log(x + 1)  # log-scale modulation
            drop_probs = torch.sigmoid(self.trace_to_prob(log_trace))


            drop_mask = torch.bernoulli(1.0 - drop_probs).unsqueeze(1)   # [B, 1, S, 1]
            attn_weights = attn_weights * drop_mask  # [B, H, S, S] masked by token-wise prob
...
class GlialMemoryModel(nn.Module):
    ...
    def forward(self, x, expected_probs, astro_trace):
        ...
        # --- Entropy from simulation output ---
        entropy = -torch.sum(sim_probs * torch.log(sim_probs + 1e-8), dim=-1, keepdim=True)  # [B, S, 1]

        # --- Comparator score ---
        comp_score = self.comparator(sim_probs, expected_probs)  # [B, S, 1]

        # --- Combine both signals ---
        uncertainty_weight = 0.5  # Tuneable
        trace_update = (1 - uncertainty_weight) * comp_score + uncertainty_weight * entropy

        # --- Astro trace update ---
        new_trace = self.decay * astro_trace + (1 - self.decay) * trace_update

        # --- Feed back into representation ---
